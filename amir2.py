#!/usr/bin/env python3
# SCRIPT FOR ADVERSARIAL PREFIX OPTIMIZATION + LOSS PLOT + INITIAL↔OPT STATS
# - Performs adversarial training of a prefix vector with DDP (multi-GPU)
# - Logs per-step training loss (avg across GPUs), saves CSV + PNG plot
# - Saves initial and optimized prefixes
# - Reports cosine similarity and Euclidean distance between initial and optimized prefixes
...
def train(gpu_id, num_gpus):
    ...
    adversarial_logits = torch.nn.Parameter(
        torch.full((NUM_PREFIX_VECTORS, vocabulary_size), 1/vocabulary_size, requires_grad=True, device=gpu_id)
    )
    ...
    optimizer = torch.optim.SGD([adversarial_logits], lr=LEARNING_RATE, maximize=False)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_questions, batch_answers in data_loader:
            ...
            # 6. LOGGING (global avg loss for this step)
            with torch.no_grad():
                ...
                if gpu_id == 0:
                    ...
                    progress_bar.set_postfix(loss=f"{loss_history[-1]:.4f}", reward=f"{torch.exp(-attack_loss)}")

        if gpu_id == 0:
            progress_bar.close()

        if epoch % 20 == 19:
            # EXPERIMENT: replace top k positions with 1
            # all other positions replaced with 0
            top_results = torch.topk(adversarial_logits, 5, dim=1).indices

            best_attack_loss = torch.inf
            best_logits = None
            for num_tries in range(100):
                with torch.no_grad():
                    adversarial_logits.data.zero_()
                    for i in range(NUM_PREFIX_VECTORS):
                        adversarial_logits[i, top_results[i][random.randrange(0, 5)]] = 1
                    tokenized_batch = skywork_tokenizer_api.prepare_steps(batch_questions, batch_answers)
                    batch_embeddings = token_embedding_layer[tokenized_batch.data["input_ids"]]
                    adversarial_prefix = adversarial_logits @ token_embedding_layer
                    ...
                    if attack_loss < best_attack_loss:
                        ...
                        best_logits = adversarial_logits.clone()
            
            with torch.no_grad():
                adversarial_logits.copy_(best_logits)

            print(f"######## LOSS AFTER DISCRETIZATION: {best_attack_loss} ########")
    ...
    if gpu_id == 0:
        ...
        opt_path = os.path.join(
            RUN_DIR,
            f"aditya1_epochs{NUM_EPOCHS}_batch{BATCH_SIZE}_nvecs{NUM_PREFIX_VECTORS}_lr{LEARNING_RATE}_size{DATASET_SIZE}.pt"
        )
