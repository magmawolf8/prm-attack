...
# configuration
from prm_attack.config import SKYWORK_MODEL_NAME, DEFAULT_STEP_TOKEN, NUM_EPOCHS, BATCH_SIZE, NUM_PREFIX_VECTORS, LEARNING_RATE, RANDOM_SEED, DATASET_SIZE, MOMENTUM, L1_LAMBDA
...
def train(gpu_id, num_gpus):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_questions, batch_answers in data_loader:
            ...
            # 3. CALCULATE LOSS
            nll_loss = -torch.log(model_output.rewards[tokenized_batch.data["reward_flags"].bool()]).mean()
            ...
            with torch.no_grad():
                ...
                if gpu_id == 0:
                    ...
                    progress_bar.set_postfix({
                        ...
                        "near0%": f"{(adversarial_logits.abs() < 1e-3).float().mean().item() * 100:.1f}",
                        "H(p)": f"{(- (probs * probs.clamp_min(1e-12).log()).sum(-1).mean()).item():.2f}",
                    })
